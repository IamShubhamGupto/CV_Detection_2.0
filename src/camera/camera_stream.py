...
class CameraVideoStream:
    ...
    def open(self, src, backend=None):
        ...
        self.framecounter += 1
        logging.warn("CAM %d - GRAB - frame %d @ time %f",
            ...
            if not (self.suspend):
                ...
                if (latest_timestamp > self.timestamp):
                    ...
                    logging.warn("CAM %d - GRAB - frame %d @ time %f",
                                 ...
                                 latest_timestamp)
                    logging.warn("CAM %d - GRAB - inter-frame diff (ms) %f",
                                  self.threadID,
                                  latest_timestamp - self.timestamp)
                    self.timestamp = latest_timestamp
                else:
                    logging.warn("CAM %d - GRAB - same timestamp skip %d",
    ...
    def read(self):
        ...
        for skip in range(1, frame_offset):
            logging.warn("CAM %d - SKIP - frame %d", self.threadID,
                         self.framecounter_last_read - frame_offset + skip)

        logging.warn("CAM %d - READ - frame %d @ time %f",
                     self.threadID, self.framecounter, self.timestamp)
    ...
    def set(self, property_name, property_value):
        ...
        self.timestamp = self.camera.get(cv2.CAP_PROP_POS_MSEC)
        self.framecounter += 1
        logging.warn("CAM %d - GRAB - frame %d @ time %f", self.threadID,
